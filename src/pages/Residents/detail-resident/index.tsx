...
const DetailResidentPage = () => {
  const { id } = useParams();
  const { data: resident, isLoading, isError } = useGetResidentById(Number(id));
    ...
    return (
      <div className="p-1.5 pt-0">
        <div className="bg-red-50 border border-red-200 p-8 rounded text-center text-red-600">
          <p className="font-medium">Không thể tải dữ liệu cư dân!</p>
        </div>
      </div>
    );
  }
        ...
        <div className="space-y-2">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10">
            ...
            <div>
              ...
              <p className="display-text">{new Date(resident.dob).toLocaleDateString("vi-VN")}</p>
            </div>
